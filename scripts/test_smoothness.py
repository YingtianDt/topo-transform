...
if __name__ == '__main__':
    ...
    display_labels = ['Human', 'Ours', 'TDANN', 'SwapOpt', 'VJEPA']
    
    # Reorder data to match display order (Human first, then models)
    label_order = ['HUMAN', 'MODEL', 'TDANN', 'SWAPOPT', 'UNOPTIMIZED']
    
    # Colors: green for Human, blue for MODEL (Ours), gray for others
    colors_moving = [HUMAN_C, MODEL_C, DEFAULT_C, DEFAULT_C, DEFAULT_C]
    colors_static = ['#8FC794', '#8DB3D6', '#9B9B9B', '#9B9B9B', '#9B9B9B']
    ...
    # Print statistics
    print("\n" + "="*50)
    print("MODEL PERFORMANCE (MOVING vs STATIC)")
    print("="*50)
    for label, mov_mean, stat_mean in zip(display_labels, moving_means, static_means):
        print(f"{label:12s} - Moving: {mov_mean:.4f}, Static: {stat_mean:.4f}, Diff: {mov_mean - stat_mean:+.4f}")